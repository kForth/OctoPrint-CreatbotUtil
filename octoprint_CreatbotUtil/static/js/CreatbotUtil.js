  ...
  function CreatbotUtilViewModel(parameters) {
    ...
    self.initializeControls = function () {
      ...
      function getInfoIcon() {
        var binding = _.sprintf(
          "popover: {placement: \"%(placement)s\", trigger: \"%(trigger)s\", title: \"%(title)s\", content: \"%(content)s\"}",
          {
            ...
            content: gettext('This setting can only be set, it cannot be read back from the firmware due to a limitation of the communication protocol. There is no way to show the current setting.')
          }
        );
        ...
        return $('<div class="slider slider-horizontal">')
          .append(
            ...
            $('<div class="tooltip top" style="top: -24px; left: -5px;">' +
                ...
                '<div class="tooltip-inner">' + max + '</div>' +
              '</div>'
            ),
            $('<input type="number" style="width: 100px; display: none;">')
              .attr("data-bind", binding),
          );
      }
      ...
      var controlDiv = $("<div>")
        ...
        .attr('data-bind', "visible: loginState.hasPermissionKo(access.permissions.CONTROL)")
        .append(
            ...
            .append(
                ...
                $('<div class="button-group">')
                  .append(
                    $('<button class="btn btn-small" disabled="">')
                      .attr("data-bind", "enable: printerState.isOperational(), click: setExtruderFanSpeed")
                      .text(gettext("Set")),
                    $('<button class="btn btn-small" disabled="">')
                      .attr("data-bind", "enable: printerState.isOperational(), click: disableExtruderFan")
              ...
              $("<div>").append(
                ...
                $('<div class="button-group">')
                  .append(
                    $('<button class="btn btn-small" disabled="">')
                    .attr("data-bind", "enable: printerState.isOperational(), click: setChamberFanSpeed")
                      .text(gettext("Set")),
                    $('<button class="btn btn-small" disabled="">')
                    .attr("data-bind", "enable: printerState.isOperational(), click: disableChamberFan")
                      .text(gettext("Disable")),
                  ),
              ),
            ),
    ...
    self.setExtruderFanSpeed = function () {
      ...
      if (speed !== NaN) {
        OctoPrint.control.sendGcodeWithParameters(
          "M6011 S%(speed)d", {speed: speed}
        );
      }
    }
